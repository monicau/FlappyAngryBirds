...
<li>Download as <a href="https://github.com/photonstorm/phaser/archive/v2.4.4.zip">zip</a> or <a href="https://github.com/photonstorm/phaser/archive/v2.4.4.tar.gz">tar.gz</a></li>
<li>Download just the build files: <a href="https://github.com/photonstorm/phaser/releases/download/v2.4.4/phaser.js">phaser.js</a> and <a href="https://github.com/photonstorm/phaser/releases/download/v2.4.4/phaser.min.js">phaser.min.js</a></li>
...
<p><code>&lt;script src=&quot;//cdn.jsdelivr.net/phaser/2.4.4/phaser.js&quot;&gt;&lt;/script&gt;</code></p>
<p>or the minified version:</p>
<p><code>&lt;script src=&quot;//cdn.jsdelivr.net/phaser/2.4.4/phaser.min.js&quot;&gt;&lt;/script&gt;</code></p>
...
<p><code>&lt;script src=&quot;https://cdnjs.cloudflare.com/ajax/libs/phaser/2.4.4/phaser.js&quot;&gt;&lt;/script&gt;</code></p>
...
<h2>Road Map</h2><p>The majority of Phaser development is now taking place on the Phaser 3 project. The Phaser 2 branch will still be supported and issues fixed, but roadmap features have been migrated over to Phaser 3.</p>
...
<h2>Phaser Nano</h2><p><a href="https://github.com/photonstorm/phaser-nano">Phaser Nano</a> is a cut-down and optimized build of Phaser designed specifically for super low file-size environments such as banner ads and interstitials. It still uses the same friendly API as Phaser but in a much smaller footprint. The current release being just 8.3KB.</p>

<p><a name="change-log"></a></p>
<h2>Change Log</h2><h2>Version 2.4.4 - &quot;Amador&quot; - 15th October 2015</h2><h3>New Features</h3><ul>
<li>Emitter.emitParticle now has 4 new optional arguments: <code>x</code>, <code>y</code>, <code>key</code> and <code>frame</code>. These allow you to override whatever the Emitter default values may be and emit the particle from the given coordinates and with a new texture.</li>
<li>Group.resetChild is a new method that allows you to call both <code>child.reset</code> and/or <code>child.loadTexture</code> on the given child object. This is used internally by <code>getFirstDead</code> and similar, but is made public so you can use it as a group iteration callback. Note that the child must have public <code>reset</code> and <code>loadTexture</code> methods to be valid for the call.</li>
<li>Group.getFirstDead, Group.getFirstAlive and Group.getFirstExists all have new optional arguments: <code>createIfNull</code>, <code>x</code>, <code>y</code>, <code>key</code> and <code>frame</code>. If the method you call cannot find a matching child (i.e. getFirstDead cannot find any dead children) then the optional <code>createIfNull</code> allows you to instantly create a new child in the group using the position and texture arguments to do so. This allows you to always get a child back from the Group and remove the need to do null checks and Group inserts from your game code. The same arguments can also be used in a different way: if <code>createIfNull</code> is false AND you provide the extra arguments AND a child is found then it will be passed to the new <code>Group.resetChild</code> method. This allows you to retrieve a child from the Group and have it reset and instantly ready for use in your game without any extra code.</li>
<li>P2.Body.removeCollisionGroup allows you to remove the given CollisionGroup, or array of CollisionGroups, from the list of groups that a body will collide with and updates the collision masks (thanks @Garbanas #2047)</li>
<li>Filter.addToWorld allows you to quickly create a Phaser.Image object at the given position and size, with the Filter ready applied to it. This can eliminate lots of duplicate code.</li>
<li>Tiled 0.13.0 added support for layer data compression when exporting as JSON. This means that any .tmx stored using base64 encoding will start exporting layer data as a base64 encoded string rather than a native array. This update adds in automatic support for this as long as the data isn't compressed. For IE9 support you'll need to use the new polyfill found in the resources folder (thanks @noidexe #2084)</li>
<li>You can now load single layer Pyxel Edit TileMaps as an atlas (thanks @joshpmcghee #2050)</li>
<li>Canvas.getSmoothingPrefix will return the vendor prefixed smoothing enabled value from the context if set, otherwise null.</li>
<li>The Random Number Generator can now get and set its state via rnd.state. This allows you to do things like saving the state of the generator to a string that can be part of a save-game file and load it back in again (thanks @luckylooke #2056 #1900)</li>
<li>Device.iOSVersion now contains the major version number of iOS.</li>
<li>The new <code>PointerMode</code> enumeration value has been added for better simple input discrimination in the future, between active pointers such as touch screens and passive pointers, such as mouse cursors (thanks @pnstickne #2062)</li>
<li>Button.justReleasedPreventsOver controls if a just-release event
on a pointer prevents it from being able to trigger an over event.</li>
<li>Button.forceOut expanded to accept a PointerMode value such that it
can be controlled per-input mode.</li>
<li>Phaser.KeyCode is a new pseudo-type used by the Keyboard class (and your code) to allow for separation of all the Keyboard constants to their own file. This stops the JSDocs from becoming 'polluted' and allows for easier future expansion (thanks @pnstickne #2118 #2031)</li>
</ul>
<h3>Updates</h3><ul>
<li>TypeScript definitions fixes and updates (thanks @clark-stevenson @milkey-mouse @timotei @qdrj @Garbanas @cloakedninjas)</li>
<li>Docs typo fixes (thanks @rwrountree @yeluoqiuzhi @pnstickne @fonsecas72 @JackMorganNZ @caryanne)</li>
<li>Math.average has been optimized (thanks @rwrountree #2025)</li>
<li>When calling GameObject.revive the <code>heal</code> method is called to apply the health value, allowing it to take into consideration a <code>maxHealth</code> value if set (thanks @bsparks #2027)</li>
<li>Change splice.call(arguments, ..) to use slice instead (thanks @pnstickne #2034 #2032)</li>
<li>BitmapData.move, moveH and moveV have a new optional <code>wrap</code> argument allowing you to control if the contents of the BitmapData are wrapped around the edges (true) or simply scrolled off (false).</li>
<li>Time.desiredFps has moved to a getter / setter.</li>
<li>Time.physicsElapsed and Time.physicsElapsedMS are no longer calculated every frame, but only when the desiredFps is changed.</li>
<li>Time.update has been streamlined and the <code>updateSetTimeout</code> and <code>updateRAF</code> methods merged and duplicate code removed.</li>
<li>Time.desiredFpsMult is a pre-calculated multiplier used in Game.update.</li>
<li>Time.refresh updates the <code>Time.time</code> and <code>Time.elapsedMS</code> values and is called automatically by Game.update.</li>
<li>DeviceButton was setting a <code>duration</code> property on itself, which went against the read only getter of duration (thanks @winstonwolff)</li>
<li>Added Node.js v4 stable to Travis config (thanks @phillipalexander #2070)</li>
<li>Optimised Canvas.getSmoothingEnabled, Canvas.setSmoothingEnabled and Canvas.setImageRenderingCrisp.</li>
<li>The Physics Editor Exporter (found in the resources folder of the repo) has had an option to prefix shape names and optimize JSON output added to it (thanks @Garbanas #2093)</li>
<li>Touch.addTouchLockCallback has a new argument <code>onEnd</code> which allows the callback to fire either on a touchstart or a touchend event.</li>
<li>The SoundManager now detects if the browser is running under iOS9 and uses a touchend callback to unlock the audio subsystem. Previous versions of iOS (and Android) still use touchstart. This fixes Apple's screw-up with regard to changing the way Web Audio should be triggered in Mobile Safari. Thanks Apple (thanks @MyCatCarlos for the heads-up #2095)</li>
<li>InputHandler.validForInput now checks if the game object has <code>input.enabled</code> set to <code>false</code> and doesn't validate it for input if that's the case.</li>
<li>The default Button.onOverMouseOnly value has changed from <code>false</code> to <code>true</code>. If you used this in your touch enabled games then please be aware of this change (#2083)</li>
<li>BitmapData.clear now automatically calls BitmapData.update at the end of it.</li>
<li>New Color stub added for the custom build process. Contains just the bare minimum of functions that Phaser needs to work. Cuts file size from 48.7KB to 7.4KB. Note: Do not stub this out if using BitmapData objects.</li>
<li>New DOM stub added for the custom build process. Contains just the bare minimum of functions that Phaser needs to work. Cuts file size from 14.8KB to 2.4KB. Note: Do not stub this out if using the full Scale Manager.</li>
<li>New Scale Manager stub added. Removes all Scale Manager handling from Phaser! But saves 75KB in the process. If you know you don't need to scale the Phaser canvas, or are handling that externally, then you can safely stub it out in a custom build.</li>
<li>Added the PIXI.PolyK, PIXI.WebGLGraphics and PIXI.CanvasGraphics files to the Graphics custom build option. They weren't used anyway and this removes an extra 40.2KB from the build size.</li>
<li>Phaser.Create no longer automatically creates a BitmapData object when it starts. It now only does it when you first make a texture or grid.</li>
<li>New Create stub added for the custom build process. Cuts file size by 8KB.</li>
<li>You can now exclude the FlexGrid from custom builds, saving 15KB.</li>
<li>The ScaleManager no longer creates a Phaser.FlexGrid if the class isn't available (i.e. excluded via a custom build)</li>
<li>Time.suggestedFps is now defaulted to <code>Time.desiredFps</code> for the first few frames until things have settled down (previously it was <code>null</code>) (thanks @noidexe #2130)</li>
<li>Text with anchor 0.5 and word wrap would have an extra space added to its width calculations, this is now adjusted for (thanks @nickryall #2052 #1990)</li>
<li>ScaleManager.getParentBounds now checks if <code>parentNode</code> has an <code>offsetParent</code> before calling <code>getBoundingClientRect</code> on it (thanks @McFarts #2134)</li>
</ul>
<h3>Bug Fixes</h3><ul>
<li>Loader.bitmapFont wouldn't automatically set the <code>atlasURL</code> value if just the key was given.</li>
<li>The Loader would put the baseURL and/or path in front of <code>data:</code> and <code>blob</code> URLs (thanks @rblopes #2044)</li>
<li>When the Text width was being calculated it would add the <code>strokeThickness</code> value twice, causing an alignment offset (thanks @nickryall #2039)</li>
<li>Sound.onEndedHandler has a fix for AudioBufferSourceNode listener memory leak (thanks @Pappa #2069)</li>
<li>Game.update could call <code>updateLogic</code> multiple times in a single frame when catching up with slow device frame rates. This would cause Tweens to advance at twice the speed they should have done (thanks @mkristo)</li>
<li>Added useCapture flags to removeEventListener in MSPointer class (thanks @pmcmonagle #2055)</li>
<li>Under setTimeOut (or when <code>forceSetTimeOut</code> was true) the Time was incorrectly setting <code>Time.timeExpected</code> causing game updates to lag (thanks @satan6 #2087)</li>
<li>Fixes edge case when TilingSprite is removed before render (thanks @pnstickne #2097 #2092)</li>
<li>Camera.setBoundsToWorld only adjusts the bounds if it exists (thanks @prudolfs #2099)</li>
<li>Keyboard.addCallbacks didn't check to see if the arguments were <code>null</code>, only if they were <code>undefined</code> making the jsdocs misleading.</li>
<li>ScaleManager.getParentBounds now takes any transforms into account to get the correct parent bounds (thanks @jdnichollsc #2111 #2098)</li>
<li>Cache.addBitmapFont now applies a default value for the x and y spacing if the arguments are omitted (thanks @nlotz #2128)</li>
<li>Removed use of the <code>tilePosition</code> property in the Phaser.Rope class as it isn't implemented and caused calls to <code>Rope.reset</code> to crash (thanks @spayton #2135)</li>
<li>ScaleMin and ScaleMax stopped working in Phaser 2.3.0 due to an incorrect transform callback scope (thanks @brianbunch #2132)</li>
</ul>
...
<ul>
<li>CanvasRenderer.mapBlendModes optimised to cut down on file size.</li>
<li>PIXI.WebGLRenderer.updateTexture now returns a boolean depending on if the texture was successfully bound to the gl context or not.</li>
<li>PIXI.WebGLSpriteBatch.renderBatch would still try and render a texture even if <code>updateTexture</code> failed to bind it. It now checks the return value from <code>updateTexture</code> and ignores failed binds.</li>
<li>WebGLRenderer.mapBlendModes optimised to cut down on file size.</li>
<li>Sprite.getBounds would report an inaccurate value if the sprite was negatively scaled (causing things like generateTexture to be cut off) (thanks @DavidAPC #2108)</li>
<li>Removed DisplayObject.transformCallback as it's a Game Object component.</li>
<li>BaseTexture.skipRender is a new boolean that can be set to skip the rendering phase in the WebGL Sprite Batch. You may want to do this if you have a parent Sprite with no visible texture (i.e. uses the internal <code>__default</code> texture) that has children that you do want to render, without causing a batch flush in the process.</li>
</ul>
